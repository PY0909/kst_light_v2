...
def parse_args():
    parser = argparse.ArgumentParser(description="Train KAFNet-ProFITi on C-MAPSS")
    ...
    parser.add_argument("--async-mode", default="mixed", choices=["none", "random", "low_rate", "block_offline", "mixed"])
...
def split_dataset(dataset, seed: int):
    ...
    valid_count = max(1, int(0.2 * len(indices)))
...
def main():
    ...
    train_set, valid_set = split_dataset(dataset, args.seed)
    ...
    checkpoint = output_dir / f"kaf_profiti_{args.subset}_{int(time.time())}.pt"
    torch.save(
        {
