...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate KAFNet-ProFITi on MetroPT-3")
    ...
    parser.add_argument("--split", default="test", choices=["valid", "test", "all"])
...
def main():
    ...
    history_len = args.history_len or int(saved_args.get("history_len", 60))
    pred_len = args.pred_len or int(saved_args.get("pred_len", 12))
    stride = args.stride or int(saved_args.get("stride", 120))
